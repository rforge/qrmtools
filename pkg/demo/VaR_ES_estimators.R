...
## Estimating the risk measures VaR and ES for (BMW, Siemens) data with various
## estimators
...
## A basic plot of each time series to check if anything is 'suspicious'
## => It's always a good idea to *plot* real data
...
## Exercise: think about this line and why it works!
...
### Graphical goodness-of-fit check for GPD

## Transform with fitted distribution function => should be U[0,1]
excess <- GPD$excess
xi.hat <- GPD$xi
beta.hat <- GPD$beta
...
## Compute historical losses (for histogram); see risk.measure()
S. <- as.numeric(tail(S, n=1)) # pick out last available stock prices ("today")
w. <- lambda * S. # weights w.
L <- loss_operator(X, w.=w.) # historical losses
summary(L) # get important statistics about the losses

## Histogram with VaR and ES estimates
...
##    distributed risk-factor changes (nu ~= 3.02); more adequate here.
