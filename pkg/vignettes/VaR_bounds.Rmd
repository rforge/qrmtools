...
plot(th, wVaR.tol/res[,"dual"], type="l", ylim=ylim,
     xlab=expression(theta), ylab="Wang's approach (straightforward) but with smaller tol for uniroot()")
```
...
We can apply this to our range of variables as before. Note that
...
if(doPDF)
    pdf(file=(file <- paste0("fig_worst_VaR_",alpha,"_hom_comparison_num_problems.pdf")),
        width=6, height=6)
plot(d, VaR[1,], type="l", ylim=ylim, log="y",
     xlab=expression(italic(d)), ylab=substitute("Worst"~VaR[a]~"for Par("*theta*
     ") margins", list(a=alpha)), col=cols[1])
...
mtext("Solid line: direct; dashed line: transformed auxiliary function",
      side=4, line=1, adj=0)
if(doPDF) dev.off()
```
