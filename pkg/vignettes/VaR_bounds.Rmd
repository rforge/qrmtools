...
A straightforward implementation of `rearrange(, sample=FALSE)` is:
```{r}
rearrange_slow <- function(X)
{
    d <- ncol(X)
...
sample=FALSE)`). To this end, we consider the following setup.
```{r}
alpha <- 0.99 # confidence level
...
for(i in seq_along(d)) {
    ...
    res[i,] <- c(system.time(rearrange_slow(X))[["elapsed"]],
                 system.time(rearrange(X, sample=FALSE))[["elapsed"]])
}
```
