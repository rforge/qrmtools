...
th <- 0.99
qrmtools:::Wang_h(0, alpha = alpha, d = d, method = method, shape = th) # NaN => uniroot() fails
## Note: Wang_h() is actually already NaN for c <= 1e-17
qrmtools:::Wang_h_aux(0, alpha = alpha, d = d, method = method, shape = th) # Inf
```
...
for(j in 1:length(theta))
    h[,j] <- sapply(c, function(c.)
        qrmtools:::Wang_h(c., alpha = alpha, d = d, method = "Wang.Par", shape = theta[j]))
z <- h
