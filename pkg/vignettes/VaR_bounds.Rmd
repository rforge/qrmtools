...
Check the objective function $h(c)$ (i.e., `Wang_h()` with numerical integration) for
the default $\theta$.
```{r}
...
Check the endpoints of the objective function (for the root-finding).
```{r}
sapply(c(0, (1-alpha)/d), function(c.)
       qrmtools:::Wang_h(c., alpha=alpha, d=d, qF=qF)) # -Inf, 0
```
$-\infty$ is not a problem for root finding (for $\theta > 1$; for $\theta <= 1$ it is `NaN`!),
but the $0$ at the right endpoint is a problem. A proper initial interval $[c_l,u_l]$
with $0<c_l<=c_u<(1-\alpha)/d$ (containing the root) is thus required.
...
## 2) Compare crude_VaR_bounds(), "Wang" (numerical), "Wang.Par", "dual", RA()

Consider the setup as before.
```{r}
alpha <- 0.99 # confidence level
d <- 8 # dimension
```

The following explains why we need (at least) a lower endpoint for the root-finding procedure
if $\theta\in(0,1]$ in the Pareto case (infinite mean).
```{r}
...
Now consider a graphical comparison between the various methods. Our setup is
as follows.
```{r}
alpha <- 0.99 # confidence level
d <- 8 # or d <- 100
...
Compute the values; all with default tolerance (see implementation). Note that
...
## 3) Run-time comparison between a straightforward implementation and the actual efficient implementation

A straightforward (but inefficient) implementation of a basic
`rearrange(, sample=FALSE, is.sorted=TRUE)` is:
```{r}
...
Here's a plot which shows the improvement in %. For larger dimensions $d$, the
improvement is very close to 100%.
```{r}
...
## 4) How `rearrange()` acts on specific matrices

To see how `rearrange()` actually proceeds, consider the following example.
```{r}
A <- matrix(c(1:4, 2*(1:4)-1, 2^(0:3)), ncol=3)
rearrange(A, tol=NULL, sample=FALSE, is.sorted=TRUE)
## => Highest-possible/optimal solution

B <- matrix(rep(1:3, 3), ncol=3)
rearrange(B, tol=NULL, sample=FALSE, is.sorted=TRUE)
## => Non-optimal solution (6 would be better and possible
##    via the rearrangement matrix(c(1,2,3, 2,3,1, 3,1,2), ncol=3))
```
To see the rearrangement steps for each column, consider using `rearrange(,
trace=TRUE)`.
...
system.time(res.ARA <- ARA(alpha, qF=qF)) # apply ARA()
stopifnot(res.ARA$converged) # check convergence
res.ARA$bounds # ARA bounds on worst VaR
system.time(res.RA <- RA(alpha, qF=qF, N=1024, abstol=0.005)) # apply RA()
